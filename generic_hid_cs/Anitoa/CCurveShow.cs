...
namespace Anitoa
{
    public class CCurveShow
    {
        ...
        public void UpdateAllcurve()
        {
            ...
            {
                int ngap = CommData.gap_loc[iy].Count;
                ...
                for (int frameindex = 0; frameindex < numWells; frameindex++)
                {
                    ...
                    {
                        if (/*CommData.gap_loc[iy].ElementAt(i) > MIN_CT + 3 && */!CommData.noGapRemove)    // 
                        {
                            int gap = CommData.gap_loc[iy].ElementAt(i);     // gap cycle location
                    ...
                    for (int i = 0; i < m_Size[iy]; i++)
                    {
                        m_yData[iy, frameindex, i] =  yData[i];
                    }
                }
            }
            ...
            for (int iy = 0; iy < MAX_CHAN; iy++)
            {
                ...
                for (int frameindex = 0; frameindex < numWells; frameindex++)
                {
                    ...
                    {                        
                        ...
                        // if (currbase > 0)        // Nov. 2020 Commented out to correct the large negative jump issue.
                            currbase /= (MIN_CT - START_CYCLE);
        ...
        public void CalculateCT()
        {
            ...
            for (int i = 0; i < numChannels; i++)
            {
                ...
                for (int j = 0; j < numWells; j++)
                {
                    ...
                    OutlierRemove(ref yData, size, i, j, OUTLIER_THRESHOLD);
                    ...
                    //============= Test code: distortion repair==============
#if GAP_REMOVE
                    int ngap = CommData.gap_loc[i].Count;

                    for (int ii = 0; ii < ngap; ii++)
                    {
                        if (CommData.gap_loc[i].ElementAt(ii) > MIN_CT + 3 && !CommData.noGapRemove)    // 
                        {
                            int gap = CommData.gap_loc[i].ElementAt(ii);     // gap cycle location
        ...
        public int curvefit(double[] x, double[] y, int well, int color, int iter, int size)
        {
            ...
            for (int j = 0; j < iter; j++)
            {
                ...
                if ((r[well, color] > 0.65 && delta_r > 0) || (r[well, color] < 0.12 && delta_r < 0)) // Zhimin: 11/19/21 Reduced r lower limit
                    delta_r = 0;
                ...
                if (r[well, color] > 0.7)
                    r[well, color] = 0.7;
                else if (r[well, color] < 0.1)  // Zhimin: 11/19/21 Reduced r lower limit
                    r[well, color] = 0.1;
            ...
            // Ct compensation for ct0 close to cycle_num, done 11/19/2021

            double orig_ct = m_CTValue[color, well];

            double t_adder = 0;
            double k_multi = 1;

            double gap = size - orig_ct;

            if (gap < 0) gap = 0;
            if(gap < 10)
            {
                t_adder = (10 - gap) * 0.3;
                k_multi = 1 + (10 - gap) * 0.043;
            }

            t[well, color] += t_adder;
            k[well, color] *= k_multi;
            r[well, color] *= k_multi;
...
//#endif
            ...
            //    if (CommData.noCt)
            //        m_bFactor[i, k] = bData[k];
            }
        }

        private void DarkCorrect2(int i)         // i is channel
        {
            double[] bData = new double[MAX_CYCL];
            int bsize = m_Size[i];
            ...
            double[] yy = new double[MIN_CT + 10];
            double[] xx = new double[MIN_CT + 10];

            if (bsize < MIN_CT + 10)
                return;

            for (int k = 0; k < MIN_CT + 10; k++)   // assuming bsize > MIN_CT + 10;
            {
                yy[k] = bData[k + 1];
                xx[k] = (double)k + 1;
            }
            ...
            var bpolyfit = new PolyFit(xx, yy, 1);
            var bfitted = bpolyfit.Fit(x);
        ...
        private void OutlierRemove(ref double[] yData, int size, int i, int j, double threshold)
        {
            ...
            for (int k = 0; k < size; k++)
            {
                if (diff[k] > threshold * stdev || diff[k] < -threshold * stdev)
                {
                    yData[k] = fitted[k];
                }
            }            
        ...
        private void PivotBase(ref double[] yData, int size, int i, int j)        
        {
            ...
            // 5-13-2020 change due to Takara, pivot less

            double slope = polyfit2.Coeff[1] * 0.5;
            double intercept = polyfit2.Coeff[0] + 1.5 * slope; // pivot at point x = 3;

            for (int k = 0; k < size; k++)
            {
                yData[k] -= intercept + (double)k * slope;
                yData[k] += m_mean[i,j];
            }

            m_stdev[i, j] = stdev2;
            m_intercept[i, j] = intercept;
            m_slope[i, j] = slope;
        }
        ...
        const double MAX_EFF_LOW = 0.7;
        const double RATIO_CREDIT_POWER = 0.1;
        ...
        public void CheckFalsePositive(int iy)
        {
            ...
            for (int frameindex = 0; frameindex < numWells; frameindex++)
            {
                ...
                if (max_k < k[frameindex, iy])
                {
                    ...
                    max_i = frameindex;
                    max_ct = m_CTValue[iy, frameindex];
                }
            }
            ...
            {
                m_Advisory[iy] = "Channel " + (iy + 1).ToString() 
                    + " signal too weak. Please change gain mode to high for this channel for the next experiment.";
            }
            ...
            for (int frameindex = 0; frameindex < numWells; frameindex++)
            {
                if (m_CTValue[iy, frameindex] < 0.1 || k[frameindex, iy] < 1)
                    continue;
            ...
            for (int frameindex = 0; frameindex < numWells; frameindex++)
            {
                ...
                double ct_diff = ct - max_ct;

                if (ct_diff < 0) ct_diff = 0;

                ratio_k *= Math.Pow(2, ct_diff * RATIO_CREDIT_POWER);      // ct diff 10 results in ratio credit double

                if (ratio_k < 0) ratio_k = 0;
                else if (ratio_k > 1) ratio_k = 1;
                ...
                confi = ratio_k * 0.94 + (ratio_eff - 0.3) * 1.43 * 0.06;       // Weighted average of 2 scores
